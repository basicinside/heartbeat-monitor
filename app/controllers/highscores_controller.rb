class HighscoresController < ApplicationController
  ...
  def provinces
    @provinces = Province.find(:all,  :select => "provinces.id, provinces.name, provinces.description, SUM(scores.score) AS score",  :conditions => ["nodes.last_seen > '#{@@last_seen}'"],
                               :joins => {:locations =>  {:users => {:nodes => :scores}}}, :group => "provinces.id, provinces.name, provinces.description", :order => 'SUM(scores.score) DESC')
  end

  def parties
    @parties = Party.find(:all,  :select => "parties.name, parties.id, parties.description, SUM(scores.score) AS score",  :conditions => ["nodes.last_seen > '#{@@last_seen}'"],
                          :joins =>  {:users => {:nodes => :scores}}, :group => "parties.name, parties.id, parties.description", :order => 'SUM(scores.score) DESC')
  end
